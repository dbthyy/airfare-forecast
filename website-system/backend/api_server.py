...
# ─── Config & Paths (Sửa lỗi FileNotFoundError trên Render) ────────────────────
BASE_DIR         = Path(os.path.dirname(os.path.abspath(__file__)))
DATA_DIR         = BASE_DIR / "data" / "raw"
DATA_DIR.mkdir(parents=True, exist_ok=True)

MODELS_PATH      = BASE_DIR / "route_models.pkl"
WEIGHTS_PATH     = BASE_DIR / "route_weights.pkl"
...
def find_existing_csv(dest_code: str, depart_date: str):
    try:
        ...
        log.error("Lỗi parse ngày tháng hoặc tìm file cache: %s", e)
        return None
...
def build_url(dest_code: str, date_str: str) -> str:
    ...
    return f"https://www.traveloka.com/vi-vn/flight/fullsearch?ap=SGN.{dest_code}&dt={d}-{m}-{y}.NA&ps=1.0.0&sc=ECONOMY"
...
def _attach_predictions(rows: list[dict]) -> list[dict]:
    ...
    try:
        ...
        start_day  = rows[0].get("start_day", "")
        
        if dest_match and start_day:
            dest_code  = dest_match.group(1)
            clean_day  = start_day.replace(" ", "_")
...
# Cấu hình CORS mở rộng cho phép nhận đủ các request từ domain GitHub Pages
...
def search_flights():
    # Bộ lọc chống sập lỗi 405/CORS Preflight cho phương thức GET/OPTIONS
    if request.method == "GET":
        return jsonify({
            "status": "active",
            "message": "API Search đang hoạt động tốt. Hãy dùng phương thức POST để truyền dữ liệu tìm kiếm."
        }), 200
...
def predict_only():
    body = request.get_json(force=True) or {}
    if "csv_path" in body:
        p = DATA_DIR.parent / body["csv_path"]
        if not p.exists():
...
def train_model():
    ...
    return jsonify({
        ...
        "message": f"Đang train từ {len(list(DATA_DIR.glob('*.csv')))} file CSV.",
    })
